...
bridged_vars = []
depths = range(0, 210, 10)

u_vars = open("1GeVunconstrainedVars.txt", "r")
for item in u_vars:
...
c_vars = open("1GeVconstrainedVars_200cm.txt", "r")
for item in c_vars:
...
for i in range(len(unconstrained_vars_trunc)):
    bridged_vars.append(unconstrained_vars_trunc[i]*unconstrained_vars_rev[i]/0.104)
